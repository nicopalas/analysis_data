...
void gold_xs(){
    ...
    // ── energy binning ────────────────────────────────────────────────────────
    const int nbins = 15;
    std::vector<double> energy_bins = buildLogBins(nbins, 10.0, 1000.0);
...
void uranium_xs(){
    ...
    // ── reference gold data ───────────────────────────────────────────────────
    std::vector<double> E_ref  = {10, 11};
    std::vector<double> sig_ref= {1.01117324, 1.00882950};
    std::vector<double> u_ref  = {0.00815660, 0.00806416};
    std::vector<double> ex_ref(E_ref.size(), 0.0);
    for (auto &s : sig_ref) s *= 1e-3;   // mb → barn
    for (auto &u : u_ref)   u *= 1e-3;
    ...
    gr_exp->SetTitle("U-197 this work (normalised);E_{n} (MeV);#sigma (barn)");
    gr_exp->SetMarkerStyle(20);
    gr_exp->SetMarkerColor(kBlue+1);
    gr_exp->SetLineColor(kBlue+1);
    gr_exp->SetLineWidth(2);
    ...
    TCanvas *c = new TCanvas("c_gold", "U-238 cross section", 900, 600);
    c->SetLogy();
    gr_exp->Draw("AP");
    gr_exp->GetXaxis()->SetTitle("E_{n} (MeV)");
    gr_exp->GetYaxis()->SetTitle("#sigma (barn)");
